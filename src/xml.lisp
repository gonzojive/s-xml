...
(define-condition unmatched-namespace-error (error)
  ((namespace :initarg :namespace :reader unmatched-namespace-error-namespace))
  (:documentation "When a namespace attached to an XML node does not have a
corresponding lisp package, this condition is signaled."))
    ...
    (flet ((intern-symbol (string package) ; intern string as a symbol in package
      ...
      (multiple-value-bind (prefix name)
        ...
        (if (or (null prefix) (string= prefix "xmlns"))
		 ...
		 (error 'unmatched-namespace-error :namespace prefix) )
             (use-package (package) (intern-symbol name package)) )))))))
                          
;;"namespace not found for prefix ~s" prefix))))))))
   ...
   (text-hook :documentation "Called when text is found"
	      ...
	      :initform #'(lambda (string seed)
			    (declare (ignore string))
                            seed))
   (stage :documentation "Indicates the stage of XML processing.  One of :preamble, :body.
When :preamble, DTD and <?xml... tags are recognized."
          :initarg :stage :initform :preamble :accessor get-stage)
   (doctype-decl :documentation "Points to a DTD for the current XML document."
                 :initarg :doctype-declaration :accessor get-doctype-declaration
                 :initform nil))
...
(defvar *report-all-whitespace* t
  "When t, namespaces are ignored like in the old version of S-XML")
        ...
        (loop
         ...
         (cond ((null char) (error (parser-error "encountered unexpected eof handling ~a" (list open-tag))))
               ((char= char #\<)
                ...
                (if (char= (peek-char nil stream nil nil) #\/)
                    (progn
                      ...
                      (when (and (or (not has-children) *report-all-whitespace*)
				 (plusp (length buffer)))
		    ...
		    ;; do NOT ignore whitespace
		      (when *report-all-whitespace*
			(setf (get-seed state)
			      (funcall (get-text-hook state) (copy-seq buffer) (get-seed state))))
